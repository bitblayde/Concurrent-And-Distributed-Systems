...
{
   int contador = Arraynp[num_hebra] + num_hebra*num_items/np;
   this_thread::sleep_for( chrono::milliseconds( aleatorio<20,100>() ));
   mtx.lock();
   cout << "producido: " << contador << " Hebra " << num_hebra << endl << flush ;
   mtx.unlock();
   cont_prod[contador] ++ ;
   Arraynp[num_hebra]++;
   return contador++ ;
...
void funcion_hebra_consumidora( ProdCons1SC * monitor, int n_hebra )
{
...
{
   cout << "-------------------------------------------------------------------------------" << endl
        << "Problema de los productores-consumidores (prod/cons Múltiples, Monitor SC, buffer FIFO). " << endl
        << "-------------------------------------------------------------------------------" << endl
        << flush ;
