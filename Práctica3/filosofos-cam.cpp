...
const int etiq_solicitar_tenedor = 0,
		  etiq_liberar_tenedor = 1,
		  etiq_sentarse = 2,
		  etiq_levantarse = 3;
...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
  	...
  	cout << "El filosofo " << id << "solicita sentarse " << endl;
  	MPI_Ssend(NULL, 0, MPI_INT, id_camarero, etiq_sentarse, MPI_COMM_WORLD);
  	
  	cout << "El filosofo " << id << " puede sentarse " << endl;
    ...
    MPI_Ssend(NULL, 0, MPI_INT, id_ten_izq, etiq_liberar_tenedor, MPI_COMM_WORLD);

    cout<< "Filósofo " <<id <<" suelta ten. der. " <<id_ten_der <<endl;
    // ... soltar el tenedor derecho (completar)
    MPI_Ssend(NULL, 0, MPI_INT, id_ten_der, etiq_liberar_tenedor, MPI_COMM_WORLD);


	// El filósofo solicita levantarse de la mesa.
...
void funcion_tenedores( int id )
{
  ...
  while ( true )
  {
  		MPI_Recv(NULL, 0, MPI_INT, MPI_ANY_SOURCE, etiq_solicitar_tenedor, MPI_COMM_WORLD, &estado);
     ...
     cout <<"Ten. " <<id <<" ha sido cogido por filo. " <<id_filosofo <<endl;
		...
		MPI_Recv(NULL, 0, MPI_INT, id_filosofo, etiq_liberar_tenedor, MPI_COMM_WORLD, &estado);
...
void funcion_camarero(){
	...
	while(true){
		...
		if(estado.MPI_TAG == etiq_levantarse){
			filo_sentados--;
			cout << "El camarero levanta al filosofo " << id_filosofo << " y hay " << filo_sentados << " filosofos sentados" << endl;
		}
		else if(estado.MPI_TAG == etiq_sentarse){
			filo_sentados++;
			MPI_Ssend(NULL, 0, MPI_INT, id_filosofo, etiq_sentarse, MPI_COMM_WORLD);
			cout << "El camarero da el turno al filosofo " << id_filosofo << " y hay " << filo_sentados << " filosofos sentados" << endl;
		}
	}
}
// ---------------------------------------------------------------------
